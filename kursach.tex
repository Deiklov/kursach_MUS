...
\]
Из $m(0)=0$ находим, что $C_0=0$
\begin{equation}\label{solve1stage}
    \left\{ {\begin{aligned}
                 & 1 = C_1+C_2, \hfill             \\
                 & \Omega_0 = C_1-C_2-u_* , \hfill \\
            \end{aligned}} \right.
    \Longrightarrow \left\{ {\begin{aligned}
                 & C_0 = 0, \hfill                           \\
                 & C_1 = \frac{1+(\Omega_0+u_*)}{2}, \hfill  \\
                 & C_2 = \frac{1-(\Omega_0+u_*)}{2} . \hfill \\
            \end{aligned}} \right.
\end{equation}
Из \eqref{solve1stage} получим
\begin{equation}\label{solvefull1stage}
    \left\{ {\begin{aligned}
                 & m_1(\tau) = u_*\tau, \hfill                                                                                \\
                 & \theta_1(\tau) = \frac{1+(\Omega_0+u_*)}{2}e^\tau+\frac{1-(\Omega_0+u_*)}{2}e^{-\tau}-C_0-u_*\tau , \hfill \\
                 & \omega_1(\tau) = \frac{1+(\Omega_0+u_*)}{2}e^\tau-\frac{1-(\Omega_0+u_*)}{2}e^{-\tau}-u_*  . \hfill        \\
            \end{aligned}} \right.
\end{equation}
